...
int main() {
  ...
  while (wb_robot_step(TIME_STEP) != -1) {
    ...
    if (pause_counter_K < 50 && pause_counter_K > 45){
      left_speed_K_front = -SPEED;
      right_speed_K_front = SPEED;
     ...
     */
    if ( pause_counter_K <= 45 && pause_counter_K > 10) {
      left_speed_K_front = SPEED;
      right_speed_K_front = SPEED;
      left_speed_K_back = SPEED;
      right_speed_K_back = SPEED;
     ...
     */
      ...
      for (i = width_K / 3; i < 2 * width_K / 3; i++) {
        for (j = height_K / 2; j < 3 * height_K / 4; j++) {
